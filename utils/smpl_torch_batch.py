...
class SMPLModel(Module):
    def __init__(self, device=torch.device('cpu'), model_path='./SMPL_NEUTRAL.pkl',
        ...
        self.J_halpe_regressor = torch.from_numpy(np.load('data/J_regressor_halpe.npy')).to(self.data_type).to(self.device)
        self.J_lsp_regressor = torch.from_numpy(np.load('data/J_regressor_lsp.npy')).to(self.data_type).to(self.device)
