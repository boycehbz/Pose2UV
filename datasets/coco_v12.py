...
class COCODataset(JointsDataset):
    ...
    '''
    def __init__(self, is_train, data_folder, name, smpl, occlusions, generator, visible_only):
        root = os.path.join(data_folder, name)
        self.dataset_dir = os.path.join(data_folder, name)
        if is_train:
            image_set = os.path.join(self.dataset_dir, 'annot/train.pkl')
        else:
            image_set = os.path.join(self.dataset_dir, 'annot/test.pkl')
        ...
        super().__init__(root, image_set, is_train, transform, visible_only)
        self.nms_thre = 1.0 #cfg.TEST.NMS_THRE
        self.image_thre = 0 #cfg.TEST.IMAGE_THRE
        self.soft_nms = False #cfg.TEST.SOFT_NMS
        self.oks_thre = 0.9 #cfg.TEST.OKS_THRE
        self.in_vis_thre = 0.2 #cfg.TEST.IN_VIS_THRE
        self.bbox_file = None #cfg.TEST.COCO_BBOX_FILE
        ...
        self.annot = self.load_pkl(image_set)
    ...
    def _load_coco_keypoint_annotation_kernal(self, ant):
        """
        ...
        """
        im_ann = ant
        height, width = im_ann['h_w']
        img_path = os.path.join(self.dataset_dir, im_ann['img_path'].replace('\\', '/'))
        ...
        # sanitize bboxes
        valid_objs = []
        for key in im_ann.keys():
            obj = im_ann[key]
            x1, y1, x2, y2 = np.array(obj['bbox'], dtype=np.float32).reshape(-1,)
            x1 = np.max((0, x1))
            y1 = np.max((0, y1))
        ...
        for obj in objs:
            ...
            for ipt in range(self.num_joints):
                joints_3d[ipt, 0] = obj['coco_joints_2d'][ipt][0]
                joints_3d[ipt, 1] = obj['coco_joints_2d'][ipt][1]
                joints_3d[ipt, 2] = 0
                t_vis = obj['coco_joints_2d'][ipt][2]
                if t_vis > 1:
                    t_vis = 1
            ...
            # vis_img('masks', mask*255)
            # vis_img('img', img)

            center, scale = self._box2cs(obj['clean_bbox'][:4])
            x1, y1 = obj['clean_bbox'][:2]
            ...
            rec.append({
                ...
                'det_pose': obj['halpe_joints_2d_pred'][:17],
